...
namespace W7\App\Model\Service\UEditor;

use W7\App;
use W7\App\Model\Logic\SettingLogic;
use W7\App\Model\Service\CdnLogic;
use W7\App\Model\Service\UploadService;
...
{
	/**
	 * @var \W7\Http\Message\Server\Request
	 */
	private $request;
	...
	private $stateMap = array( //上传状态映射表，国际化用户需考虑此处数据的国际化
		...
		'ERROR_SIZE_EXCEED' => '仅支持上传5M以内的图片，请压缩后重新上传',
	...
	private function upFile()
	{
		...
		$file = $uploadFile = $this->request->getUploadedFiles()[$this->fileField];
...
//		$path = UploadService::save($uploadFile, $dir);
//		if ($path === false) {
//			$this->stateInfo = $this->getStateInfo('ERROR_FILE_MOVE');
//		} else {
//			$this->fullName = get_full_path($path);
//			$this->stateInfo = $this->stateMap[0];
//		}
		...
		try {
			$url = CdnLogic::instance()->channel(SettingLogic::KEY_COS)->uploadFile($filename, $tmpPath);
		} catch (\Throwable $e) {
			$this->stateInfo = $this->getStateInfo('ERROR_FILE_MOVE');
		}
